...
even when the file is larger than `large-file-warning-threshold'.")
  (defun daftemacs/abort-if-file-too-large--check-ok-large-file-types (orig-fun size op-type filename &rest args)
  ...
  (advice-add 'abort-if-file-too-large :around #'daftemacs/abort-if-file-too-large--check-ok-large-file-types))
  ...
  (add-hook 'vterm-mode-hook (lambda ()
                               ...
                               (let ((buffer (current-buffer)))
                                 (when (string-prefix-p "*vterminal - dedicated" (buffer-name buffer))
                                   (hide-mode-line-mode)
                                   (define-key vterm-mode-map (kbd "C-x 1") #'(lambda ()
                                                                                (interactive)
                                                                                (message "`delete-other-windows` is disabled in the vterminal dedicated window.")))
                                   (define-key vterm-mode-map (kbd "C-x 2") #'(lambda ()
                                                                                (interactive)
                                                                                (message "`split-window-below` is disabled in the vterminal dedicated window.")))
                                   (define-key vterm-mode-map (kbd "C-x 3") #'(lambda ()
                                                                                (interactive)
                                                                                (message "`split-window-right` is disabled in the vterminal dedicated window.")))
...
(global-set-key (kbd "C-c u t") 'gt-translate)
...
(global-set-key (kbd "C-c u $") 'stock-tracker-start)
